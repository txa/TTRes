...
All work packages are do be formally developed in/with Agda (``theory'' and tool support)
 ...
 \begin{enumerate}
 \item Define a presheaf semantics for core Type Theory.
 \item Model the monotone resources in the above.
 \item Examples using local state (see HTT, queues).
 ...
 \item More examples using other monotone resources (thread, transaction based memory (STM)).
 \item Investigate other application of presheaf type theory like HOAS.
 \item Establish reasoning principles for monotone resources (Separation/Hoare Logic, see HTT)
 \item Effective tool support for using the above reasoning principles.
 \end{enumerate}
