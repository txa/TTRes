...
\title{Presheaf Type Theory for  monotone Resources\\ \emph{---~Case
  for Support~---}}
...
\br{Need to polish track record. txa longer, br shorter.}
...
Bernhard Reus is a Senior Lecturer in Computer Science at the University of Sussex with special interest in program semantics and  logics.  He is a member of the Foundations of Software Systems group in the Department of Informatics at Sussex with currently  6 members of faculty, 2 postdocs and 6 PhD students. His recent research contributions have been in the area of program verification and semantics for  programming languages and logics, in particular languages with stored procedures. 
He led a small team to build a verification tool for C-like programs that supports (semi-)automatic reasoning about stored procedures (function pointers). In co-operation with colleagues in Saarbr\"ucken, Copenhagen, Oxford and Paris he developed new models for the elegant reasoning about recursive predicates indexed by recursively defined worlds. The models developed are presheaf-models with extra structure which is relevant to the proposed project.
 ...
 Reus has published over 50 papers  in international conferences (LICS, POPL, CSL, ESOP, ICALP, FOSSACS, VMCAI) and journals (TCS, MSCS, JFP, LMCS). He co-authored a frequently cited new denotational  semantics for the call-by-name  $\lambda$-calculus with control operators and a derivation of an interpreter for this functional language  (Krivine's machine)   \cite{RS7} and was involved in the development of
  a  structural operational semantics of multi-threaded Java  \cite{R2} and  a  OCL like Hoare-logic for a sequential Java-like language \cite{R12}. 
    He has experience in formal semantics, domain theory, type theory and its categorical semantics, formalisation of proofs in type theoretical systems (Lego, precursor of the Agda system as used in the project). 
 His work on formalizations of Synthetic Domain Theory   has been generalised to allow for more models in \cite{RS8} which in turn  has been generalised further by other researchers.

Reus  co-chaired and co-organised events  in both areas, notably   the now well established \emph{Domains}   and  \emph{Types}  workshops (in 1997, 2008 and in 1998, respectively). He has been the PC Chair for Domains IX (Sussex, 2008), and served on the PC for Domains X (2011) and Classical Logic and Computation (Brno, 2010). 
%Reus has supervised two PhD students and externally examined two PhD students.  
...
Reus was the sole PI on the recent EPSRC project     \emph{From Reasoning Principles for Function Pointers To Logics for Self-Configuring Programs}
      ...
      One outcome is a (semi-)automatic theorem prover, Crowfoot \cite{R3}, that allows one to write C-like programs with stored procedures (function pointers), specify them using pre- and postconditions and automatically prove that the procedures meet the specification using just some annotations.
...
a new  logic for a  simple  imperative while-language where procedures  are first class data that can be stored away and updated at runtime,  but  not dynamically allocated   \cite{RS11}.  
 He  also obtained numerous  smaller grants from the DAAD, Nuffield Foundation,  EPSRC, and the London Mathematical Society, respectively, that funded   research visits and the organisation of events at Sussex.
  
 %

\subsection*{Track record of co-operation between the investigators}
The two PIs have  collaborated once before in the area of type theory, developing 
  an elegant definition of $\lambda$-terms using a generalized form of polymorphism and the CSL paper\cite{R15} has a citation found of 102 (on Google Scholar).
  At the time they also  co-organised and co-chaired   the TYPES workshop in  Kloster Irsee near Munich.

\subsection*{The host institutions}
\todo{what do we write here?}
 \newpage
...
\emph{Dependent types} are types which refer to  and   depend on
...
Further international leaders in the field  are G.~Morrisett (Harvard) and S.~Weirich (UPenn) in the US (see Trellys project), W.~Swierstra (Utrecht, Netherlands), T.~Streicher (Darmstadt) and M.~Hofmann \& A.~Abel (LMU Munich) in Germany, and the programming logic research group in Chalmers (Gothenburg, Sweden).
...
\todo{visiting researchers?  Who shall we nominate?
Wouter Swierstra
Greg Morrissett
Thomas Streicher 
 Jeremy Gibbons?
...
\todo{
We should then say which work packages they will contribute for. Project partners should send letter of support }

 
  \subsection*{Research Hypothesis and Objectives}
 While it is clear in principle that any dependently typed program can be given a presheaf semantics and that with this interpretation monotone resources can be given a functional specification, it is not yet clear how to use this idea in practice. What does it mean to interpret a given program or proof  in a presheaf semantics? How can objects in this presheaf world interact with ordinary objects? And how can we combine different notion of monotone effects which lead to different presheaf interpretations in one program?
...
Objectives
\begin{itemize}
\item Show that the presheaf semantics of Type Theory can be used to
  model monotone resources.
\item Define a syntactic extension of Type Theory which makes it
  possible to interpret type theoretic programs wrt.\ to different presheaves.
\item Implement this  as an extension to the Agda system.
...
\br{Refine and add timeline info and novelty info.}
...
objective but also as the vehicle carrying out the research. That is we
...
\txa{Milestones, more detail, how measure success? Management (meetings)?}
...
We detail the work packages. Before we need to point out that we use the local state monad as guiding example, that we use presheaf semantics as we know it can deal with such monotone resource already. 
 ...
 \begin{enumerate}
 \item Define a presheaf semantics for DTP. (Nottingham/Sussex)
 \item Model local state resources using the presheaf semantics (from above). (Nottingham/Sussex)
 \item Examples using local state (see HTT, queues). (Nottingham/Sussex).
 \item Develop \emph{Presheaf Type Theory}. (Nottingham/Sussex)
 \item Implement \emph{Presheaf Type Theory}.(Nottingham)
  \item Investigate combination of various resources and resource models. (Nottingham)
 \item More examples using other monotone resources (thread, transaction based memory (STM)). (Sussex)
 \item Investigate other application of presheaf type theory like HOAS. (Nottingham)
 \item Establish reasoning principles for monotone resources (Separation/Hoare Logic, see HTT) (Sussex)
 \item Effective tool support for using the above reasoning principles. (Sussex)
 \end{enumerate}
 
\subsection*{References}

\txa{add and include bib file from previous DTP project}
\br{add}
 ...
 \section*{Work Plan}
All work packages are do be formally developed in/with Agda (``theory'' and tool support).
...
\br{chartification, match with milestones}
\br{overview who does what}
\newpage


\section*{Justification of Resources}

\todo{still to be completed}

\subsection{Directly Incurred Costs}
\paragraph*{Staff} 

 We request two Research Assistants (scale 6 or 7(?)) to help with ? \todo{need to make twos fit with the workplan.}
...
\todo{still do be done. Any ideas? Activities will include a workshop, making agra code and extension available via a project webpage, other ideas?}
